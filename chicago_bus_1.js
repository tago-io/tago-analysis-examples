...
const LOCATIONS = [{
    ...
    "max_speed": 45,
    "break": (_.random(true, false) ? true : false)
}, {
    "id": 2,
...
async.parallel({'serie': get_serie, 'id': get_inc_id}, function (err, result) {
    ...
    bus_bucket("speed").insert({"speed": _.random(0, bus.max_speed), "serie": result.serie});
    if (bus.break) {
        bus_bucket("break_pressed").insert({"break_pressed": bus.break, "serie": result.serie});
    }
});
